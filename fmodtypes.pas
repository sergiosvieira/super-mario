...
type
  ...
  TFSoundStreamCallback = function(Stream: PFSoundStream; Buff: Pointer;
    Length, Param: integer): bytebool; stdcall;
  TFSoundDSPCallback = function(OriginalBuffer: Pointer;
    NewBuffer: Pointer; Length, Param: integer): Pointer; stdcall;
  ...
  TFSoundReadCallback = function(Buffer: Pointer; Size: cardinal;
    Handle: cardinal): cardinal; stdcall;
  TFSoundSeekCallback = procedure(Handle: cardinal; Pos: cardinal;
    Mode: byte); stdcall;
  ...
  TFMetaDataCallback = function(Name: PChar; Value: PChar;
    userdata: integer): bytebool; stdcall;
...
const
  ...
  FSOUND_XADPCM = $00400000;
  // For XBOX only - Contents are compressed as XADPCM  */
  FSOUND_VAG = $00800000;
  // For PS2 only - Contents are compressed as Sony VAG format */
  FSOUND_NONBLOCKING = $01000000;
  // For FSOUND_Stream_OpenFile - Causes stream to open in the background and not block the foreground app - stream plays only when ready.
...
const
  ...
  // special 'channel' ID for all channel based functions that want to alter the global FSOUND software mixing output
...
type
  TFSoundReverbProperties = record          // MIN     MAX    DEFAULT DESCRIPTION
    ...
    // 0       25     0       sets all listener properties (win32 only)
    EnvSize: single;
    // 1.0     100.0  7.5     environment size in meters (win32 only)
    EnvDiffusion: single;
    // 0.0     1.0    1.0     environment diffusion (win32/xbox)
    Room: integer;
    // -10000  0      -1000   room effect level (at mid frequencies) (win32/xbox)
    RoomHF: integer;
    ...
    Flags: cardinal;
    // FSOUND_REVERB_PROPERTYFLAGS - modifies the behavior of above properties (win32 only)
  end;
// [STRUCT_END]
...
const
  FSOUND_REVERBFLAGS_DECAYTIMESCALE = $00000001;
  // EnvironmentSize affects reverberation decay time
  FSOUND_REVERBFLAGS_REFLECTIONSSCALE = $00000002;
  // EnvironmentSize affects reflection level
  FSOUND_REVERBFLAGS_REFLECTIONSDELAYSCALE = $00000004;
  // EnvironmentSize affects initial reflection delay time
  FSOUND_REVERBFLAGS_REVERBSCALE = $00000008;
  // EnvironmentSize affects reflections level
  FSOUND_REVERBFLAGS_REVERBDELAYSCALE = $00000010;
  // EnvironmentSize affects late reverberation delay time
  FSOUND_REVERBFLAGS_DECAYHFLIMIT = $00000020;
  // AirAbsorptionHF affects DecayHFRatio
  FSOUND_REVERBFLAGS_ECHOTIMESCALE = $00000040;
  // EnvironmentSize affects echo time
  FSOUND_REVERBFLAGS_MODULATIONTIMESCALE = $00000080;
  // EnvironmentSize affects modulation time
  FSOUND_REVERB_FLAGS_CORE0 = $00000100;
  // PS2 Only - Reverb is applied to CORE0 (hw voices 0-23)
  FSOUND_REVERB_FLAGS_CORE1 = $00000200;
  // PS2 Only - Reverb is applied to CORE1 (hw voices 24-47)
  FSOUND_REVERBFLAGS_DEFAULT =
    FSOUND_REVERBFLAGS_DECAYTIMESCALE or FSOUND_REVERBFLAGS_REFLECTIONSSCALE or
    FSOUND_REVERBFLAGS_REFLECTIONSDELAYSCALE or
    FSOUND_REVERBFLAGS_REVERBSCALE or
    FSOUND_REVERBFLAGS_REVERBDELAYSCALE or
    FSOUND_REVERBFLAGS_DECAYHFLIMIT or
...
type
  TFSoundReverbChannelProperties = record   // MIN     MAX    DEFAULT
    ...
    Room: integer;
    // -10000  1000   0       room effect level (at low and mid frequencies) (win32/xbox)
    RoomHF: integer;
...
const
  FSOUND_REVERB_CHANNELFLAGS_DIRECTHFAUTO = $01;
  // Automatic setting of 'Direct'  due to distance from listener
  FSOUND_REVERB_CHANNELFLAGS_ROOMAUTO = $02;
  // Automatic setting of 'Room'  due to distance from listener
  FSOUND_REVERB_CHANNELFLAGS_ROOMHFAUTO = $04;
  // Automatic setting of 'RoomHF' due to distance from listener
  FSOUND_REVERB_CHANNELFLAGS_DEFAULT =
    FSOUND_REVERB_CHANNELFLAGS_DIRECTHFAUTO or
...
const
  FSOUND_INIT_USEDEFAULTMIDISYNTH = $01;
  // Causes MIDI playback to force software decoding.
  FSOUND_INIT_GLOBALFOCUS = $02;
  // For DirectSound output - sound is not muted when window is out of focus.
  FSOUND_INIT_ENABLESYSTEMCHANNELFX = $04;
  // For DirectSound output - Allows FSOUND_FX api to be used on global software mixer output!
  FSOUND_INIT_ACCURATEVULEVELS = $08;
  // This latency adjusts FSOUND_GetCurrentLevels, but incurs a small cpu and memory hit
  FSOUND_INIT_PS2_DISABLECORE0REVERB = $10;
  // PS2 only - Disable reverb on CORE 0 to regain SRAM
  FSOUND_INIT_PS2_DISABLECORE1REVERB = $20;
  // PS2 only - Disable reverb on CORE 1 to regain SRAM
  FSOUND_INIT_PS2_SWAPDMACORES = $40;
  // PS2 only - By default FMOD uses DMA CH0 for mixing, CH1 for uploads, this flag swaps them around
  FSOUND_INIT_DONTLATENCYADJUST = $80;
  // Callbacks are not latency adjusted, and are called at mix time.  Also information functions are immediate
  FSOUND_INIT_GC_INITLIBS = $100;
  // Gamecube only - Initializes GC audio libraries
  FSOUND_INIT_STREAM_FROM_MAIN_THREAD = $200;
...
type
  TFSoundTOCTag = record
    ...
    // The string "TOC" (4th character is 0), just in case this structure is accidentally treated as a string.
